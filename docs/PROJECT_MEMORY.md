...
- `Etapa 3 - Mapa greybox Silo-7 e metadados espaciais` validada manualmente no Roblox Studio e aceita pelo usuario em 2026-03-06.

## Observacoes importantes

- O projeto usa `JSON + Luau bridge` como fonte de verdade para configuracoes estaticas.
- O sistema de mapa agora suporta `GeneratedConfig` como fallback e `StudioManual` como override, via root no `Workspace` com `NukeAssaltManualMap = true`.
